...
const formRef = ref()
const form = reactive({
  openCloseTime: props.center.openCloseTime,
  name: props.center.name,
  location: props.center.location,
  coordinates: props.center.coordinates,
  image: null,
})
...
function submitForm() {
  formRef.value.validate().then(() => {
    ...
    store.updateCenter(props.center._id, formData, props.modalKey)
  }).catch((error) => {
...
onMounted(() => {
  // Если openCloseTime — это строка вроде "09:00 - 22:00", преобразуем в массив
  if (typeof form.openCloseTime === 'string') {
    const [start, end] = form.openCloseTime.split(' - ')
    form.openCloseTime = [start, end]
  }
})
</script>
...
<template>
  <a-card class="w-[500px] border-none px-2">
    ...
    fy-end items-center gap-2 mt-4">
      <a-button
          @click="submitForm"
          :loading="loadingUrl.has('update-building')"
          size="large"
          type="primary"
          class="!px-8 w-full"
      >
