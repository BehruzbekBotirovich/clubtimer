<template>
  <a-col v-for="el in centers" :key="el._id" :xs="24" :md="12" :lg="6">
    <a-card
      class="rounded-xl shadow-lg overflow-hidden border relative border-gray-200"
      :hoverable="false"
    >
      <template #cover>
        <a-image
          alt="example"
          :src="URL + el.image_url"
      ...
      <div class="text-base">
        ...
        <p class="text-gray-700 mb-2 flex items-center gap-1">
          ...
          <a-typography-link
            :href="`https://www.google.com/maps?q=${el.coordinates[0]},${el.coordinates[1]}`"
            target="_blank"
          >
            Haritada ochish
          </a-typography-link>
      ...
      <a-dropdown :trigger="['click']" v-if="user.role === 'admin'">
        ...
        <template #overlay>
          <a-menu class="w-fit">
            ...
            <a-menu-item key="0">
              <a-popconfirm
                :title="$t('center.center_delete_confirm')"
                class="flex items-center gap-2"
                :ok-text="$t('header_component.yes')"
                :cancel-text="$t('header_component.no')"
                @confirm="deleteAccount(el._id)"
                placement="left"
              >
                <icon-delete />
                <span>{{ $t('delete') }}</span>
...
const URL =
  import.meta.env.VITE_APP_BASE_URL || 'https://clubtimer-back.onrender.com'
const user = useUser()
